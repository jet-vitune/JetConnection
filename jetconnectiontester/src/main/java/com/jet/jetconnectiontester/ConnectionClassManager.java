...
public class ConnectionClassManager {
  ...
  public synchronized void addBandwidth(long bytes, long timeInMs) {
    ...
    bandwidth = (bytes) * 1.0 / (timeInMs) * BYTES_TO_BITS;
  ...
  private void notifyListeners(double bandwidth) {
    int size = mListenerList.size();
    for (int i = 0; i < size; i++) {
      mListenerList.get(i).onBandwidthStateChange(mCurrentBandwidthConnectionQuality.get(), bandwidth);
    }
  }
}